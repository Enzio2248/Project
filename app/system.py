...
class System:
    ...
    def authenticate(self, email, password):
        ...
        for staff in self.__staff_list:
            if staff.email == email and staff.check_password(password):
        ...
        # -------- MANAGER --------
        for manager in self.__manager_list:
            if manager.email == email and manager.check_password(password):

                return {
                    "role": "manager",
                    "message": "Manager login success",
                    "staff_id": manager.staff_id,
                    "name": manager.staff_name
                }
    ...
    def add_residence(self, residence, manager_id):
        ...
        if len(residence.room_list) == 0:
            raise HTTPException("Warning: Adding a residence with no rooms")
    ...
    def ban_user(self, manager_id, user_id):
        ...
        manager = next((m for m in self.__manager_list if m.staff_id == manager_id), None)
        if not manager:
